...
export const FinanceTracker: React.FC<FinanceTrackerProps> = ({ state, setState }) => {
    ...
    return (
        <div style={{ height: '100%', overflow: 'auto', background: '#fff', padding: '48px 64px', fontFamily: "'Inter', sans-serif" }}>
            ...
            <p style={{ fontSize: 13, color: '#999', marginBottom: 32, fontWeight: 500 }}>Controle de entradas e saídas</p>

            {/* Summary */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 40 }}>
                <div style={{ padding: 24, background: '#000', color: '#fff', borderRadius: 16 }}>
                ...
                <div style={{ padding: 24, background: '#fafafa', borderRadius: 16, border: '1px solid #eee' }}>
                    <div style={{ fontSize: 11, color: '#aaa', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 4 }}>Entradas</div>
                    <div style={{ fontSize: 28, fontWeight: 900 }}>R$ {totalIncome.toFixed(2)}</div>
                </div>
                <div style={{ padding: 24, background: '#fafafa', borderRadius: 16, border: '1px solid #eee' }}>
                    <div style={{ fontSize: 11, color: '#aaa', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 4 }}>Saídas</div>
                    <div style={{ fontSize: 28, fontWeight: 900 }}>R$ {totalExpense.toFixed(2)}</div>
                </div>
            </div>

            {/* Input */}
            <div style={{ display: 'flex', gap: 8, marginBottom: 32, padding: '12px 16px', background: '#fafafa', borderRadius: 12, border: '1px solid #e5e5e5', flexWrap: 'wrap' }}>
                ...
                <button onClick={addEntry} style={{ padding: '8px 20px', background: '#000', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>+</button>
            </div>

            {/* Entries List */}
            {state.finances.length === 0 && <p style={{ color: '#ccc', fontSize: 13, fontStyle: 'italic' }}>Nenhum lançamento ainda.</p>}
            {state.finances.map(entry => (
                <div key={entry.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 0', borderBottom: '1px solid #f5f5f5' }}>
                    <div style={{
                    ...
                    {editingId === entry.id ? (
                        <>
                            <input value={editDesc} onChange={e => setEditDesc(e.target.value)}
                                style={{ flex: 1, background: '#fafafa', border: '1px solid #eee', borderRadius: 6, padding: '4px 8px', fontSize: 13, outline: 'none', fontFamily: "'Inter'" }} />
                            <input value={editAmount} onChange={e => setEditAmount(e.target.value)} type="number"
                                style={{ width: 80, background: '#fafafa', border: '1px solid #eee', borderRadius: 6, padding: '4px 8px', fontSize: 13, outline: 'none', fontFamily: "'Inter'", textAlign: 'right' }} />
                            <button onClick={() => saveEdit(entry.id)} style={{ background: '#000', color: '#fff', border: 'none', borderRadius: 6, padding: '4px 12px', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>✓</button>
                        </>
                    ) : (
                        <>
                            <span style={{ flex: 1, fontSize: 14, fontWeight: 500 }} onClick={() => startEdit(entry)} >{entry.description}</span>
                            <span style={{ fontSize: 11, color: '#aaa', marginRight: 8 }}>{entry.category}</span>
                            <span style={{ fontSize: 14, fontWeight: 800, minWidth: 80, textAlign: 'right' }}>
                                {entry.type === 'expense' ? '−' : '+'} R$ {entry.amount.toFixed(2)}
                            </span>
                            <button onClick={() => removeEntry(entry.id)} style={{ background: 'none', border: 'none', color: '#ddd', cursor: 'pointer', fontSize: 14 }}
