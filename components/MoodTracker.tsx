...
export const MoodTracker: React.FC<MoodTrackerProps> = ({ state, setState }) => {
    ...
    return (
        <div style={{ height: '100%', overflow: 'auto', background: '#fff', padding: '48px 64px', fontFamily: "'Inter', sans-serif" }}>
            ...
            <p style={{ fontSize: 13, color: '#999', marginBottom: 40, fontWeight: 500 }}>Como você está se sentindo?</p>

            {/* Today's Mood */}
            <div style={{ marginBottom: 48, padding: 32, background: '#000', borderRadius: 20, color: '#fff' }}>
            ...
            {monthMoods.length > 0 && (
                <div style={{ marginTop: 32, display: 'flex', gap: 16 }}>
                    {MOODS.map(m => {
                        ...
                        return count > 0 ? (
                            <div key={m.key} style={{ padding: '12px 16px', background: '#fafafa', borderRadius: 10, border: '1px solid #eee', textAlign: 'center' }}>
