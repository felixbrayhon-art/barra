...
export const CollectionsView: React.FC<CollectionsViewProps> = ({ state, setState }) => {
    ...
    return (
        <div style={{ height: '100%', overflow: 'auto', background: '#f0f2f5', padding: '48px 64px', fontFamily: "'Inter', sans-serif" }}>
            ...
            <p style={{ fontSize: 13, color: '#8b92a5', marginBottom: 32, fontWeight: 500 }}>Organize listas e ideias por tema</p>
                ...
                <div style={{ width: 240, flexShrink: 0 }}>
                    <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
                        <input value={newName} onChange={e => setNewName(e.target.value)} onKeyDown={e => e.key === 'Enter' && addCollection()}
                            placeholder="Nova coleção..." style={{ flex: 1, padding: '10px 12px', background: '#fff', border: '1px solid #e0e3ea', borderRadius: 8, fontSize: 13, outline: 'none', fontFamily: "'Inter'" }} />
                        <button onClick={addCollection} style={{ padding: '10px 16px', background: '#1e2740', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>+</button>
                    </div>
                    ...
                    {state.collections.length === 0 && <p style={{ fontSize: 12, color: '#b5bcc9', fontStyle: 'italic', padding: 8 }}>Crie sua primeira coleção</p>}
                </div>
                        ...
                        <>
                            ...
                            <div style={{ display: 'flex', gap: 8, marginBottom: 24 }}>
                                <input value={newItemText} onChange={e => setNewItemText(e.target.value)} onKeyDown={e => e.key === 'Enter' && addItem()}
                                    placeholder="Adicionar item..." style={{ flex: 1, padding: '10px 14px', background: '#fff', border: '1px solid #e0e3ea', borderRadius: 8, fontSize: 14, outline: 'none', fontFamily: "'Inter'" }} />
                                <button onClick={addItem} style={{ padding: '10px 20px', background: '#1e2740', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 700, cursor: 'pointer' }}>+</button>
                            </div>
                            {collectionEntries.map(entry => (
                                <div key={entry.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 0', borderBottom: '1px solid #edf0f4' }}>
                                    <span style={{ fontFamily: 'monospace', color: '#b5bcc9' }}>—</span>
                                    ...
                                    <button onClick={() => removeItem(entry.id)}
                                        style={{ background: 'none', border: 'none', color: '#c5cad5', cursor: 'pointer', fontSize: 14 }}
                                        onMouseOver={e => (e.currentTarget.style.color = '#000')} onMouseOut={e => (e.currentTarget.style.color = '#ddd')}>✕</button>
                                </div>
                            ))}
                            {collectionEntries.length === 0 && <p style={{ color: '#b5bcc9', fontSize: 13, fontStyle: 'italic' }}>Coleção vazia. Adicione itens acima.</p>}
                        </>
                    ) : (
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: '#c5cad5', fontSize: 14 }}>
