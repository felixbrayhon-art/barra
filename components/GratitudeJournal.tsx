...
export const GratitudeJournal: React.FC<GratitudeJournalProps> = ({ state, setState }) => {
    ...
    return (
        <div style={{ height: '100%', overflow: 'auto', background: '#fff', padding: '48px 64px', fontFamily: "'Inter', sans-serif" }}>
            ...
            <p style={{ fontSize: 13, color: '#999', marginBottom: 40, fontWeight: 500 }}>Pelo que você é grato(a) hoje?</p>

            {/* Today's Input */}
            <div style={{
                padding: 32, background: '#000', borderRadius: 20, color: '#fff', marginBottom: 48
            }}>
                ...
                {todayEntry ? (
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                        <textarea value={inputText} onChange={e => setInputText(e.target.value)}
                            ...
                            style={{
                                flex: 1, background: '#111', color: '#fff', border: '1px solid #333', borderRadius: 12,
                                padding: '12px 16px', fontSize: 14, outline: 'none', resize: 'none', fontFamily: "'Inter'"
                            }} />
                        <button onClick={addGratitude}
                            style={{ padding: '12px 24px', background: '#fff', color: '#000', border: 'none', borderRadius: 12, fontWeight: 800, fontSize: 14, cursor: 'pointer', alignSelf: 'flex-end' }}>
                            Salvar
                        </button>
                    </div>
                )}
            ...
            {sorted.length === 0 && <p style={{ color: '#ccc', fontSize: 13, fontStyle: 'italic' }}>Nenhum registro de gratidão ainda.</p>}
            {sorted.map(entry => (
                <div key={entry.date} style={{ padding: '16px 0', borderBottom: '1px solid #f5f5f5' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        <span style={{ fontSize: 11, color: '#aaa', fontWeight: 700, textTransform: 'uppercase' }}>{formatDate(entry.date)}</span>
                        <div style={{ display: 'flex', gap: 8 }}>
                            <button onClick={() => startEdit(entry.date, entry.text)}
                                style={{ background: 'none', border: 'none', color: '#ddd', cursor: 'pointer', fontSize: 11 }}
                    ...
                    {editingDate === entry.date ? (
                        <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                            <input value={editText} onChange={e => setEditText(e.target.value)}
                                style={{ flex: 1, padding: '8px 12px', background: '#fafafa', border: '1px solid #eee', borderRadius: 8, fontSize: 13, outline: 'none', fontFamily: "'Inter'" }} />
                            <button onClick={saveEdit}
                                style={{ background: '#000', color: '#fff', border: 'none', borderRadius: 8, padding: '8px 16px', fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>✓</button>
                        </div>
                    ) : (
